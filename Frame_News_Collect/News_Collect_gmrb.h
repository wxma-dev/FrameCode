 ...
 *  【新闻搜集系统】
 *  真实的ip:
 *  http://epaper.gmw.cn/gmrb/html/2018-08/24/nw.D110000gmrb_20180824_1-01.htm
 *
 *  解析获取：
 *  http://epaper.gmw.cn/gmrb/html/2018-08/24/nw.D110000gmrb_20180824_${某版本的第几条新闻}-${版面序号}.htm
 *
 *  使用boost库，来进行开发，不管是哪个平台都没有问题的。
 *
 *  新闻搜索
 */
class CReadNewspaper_GMRB: public CReadNewspaper
{
    public:
    ...
    public:
        ...
        */
        string Get_NextReading_NewspaperAdreess()
        {
            ...
            sprintf( sBuf_Tmp, 
                   "http://epaper.gmw.cn/gmrb/html/%4.4d-%02.2d/%02.2d/nw.D110000gmrb_%4.4d%02.2d%02.2d_%d-%02.2d.htm", 
        ...
        string Get_NextReading_File()
        {
            ...
            sprintf( sBuf_Tmp, 
                   "D110000gmrb_%4.4d%02.2d%02.2d_%d-%02.2d.htm", 
                   oNewsPaper.iYear_Newspaper,  oNewsPaper.iMon_Newspaper, oNewsPaper.iDate_Newspaper,
                   this->iXh_Bm_Newspaper, this->iBm_Newspaper );
        ...
        void ReadNewsPaper()
        {
            ...
            while ( 1 )
            {
                ...
                if( 0 == this->Get_Something_NewspaperAdreess_ToFile() )
                {
                    ...
                    char sTmp_Cmd[4096];
                    memset( sTmp_Cmd, 0x00, sizeof( sTmp_Cmd ) );
                    sprintf( sTmp_Cmd, "sed \":a;N;s/<\\/P>\r\\n<P>/<\\/P><P>/g;$!ba\" %s > %s.tmp ; cp %s.tmp %s; sed -i '/content\=content\.replace/d' %s;rm -rf %s.tmp;", 
                             this->sNextRead_File, 
                             this->sNextRead_File,
                             this->sNextRead_File,
                             this->sNextRead_File,
...
char * CReadNewspaper_GMRB::sPaper_Name = "光明日报" ;
