 ...
 *  【新闻搜集系统】
 *  真实的ip:
 *  http://zqb.cyol.com/html/2018-08/24/nw.D110000zgqnb_20180824_1-01.htm
 *
 *  解析获取：
 *  http://zqb.cyol.com/html/2018-07/31/nw.D110000zgqnb_20180824_${某版本的第几条新闻}-${版面序号}.htm
 *
 *  使用boost库，来进行开发，不管是哪个平台都没有问题的。
 *
 *  新闻搜索
 */
class CReadNewspaper_ZGQNB: public CReadNewspaper
{
    public:
        CNewspaper oNewsPaper;

        CReadNewspaper_ZGQNB()
        {
            CNewspaper a( sPaper_Name );
            oNewsPaper = a;

            /* 此处增加include相关的过滤信息，如果有变动，需要则重新编译-------------start------------- */
            #include "Gl_zgqnb.cfg"
    ...
    public:
        ...
        */
        string Get_NextReading_NewspaperAdreess()
        {
            ...
            sprintf( sBuf_Tmp, 
                   "http://zqb.cyol.com/html/%4.4d-%02.2d/%02.2d/nw.D110000zgqnb_%4.4d%02.2d%02.2d_%d-%02.2d.htm", 
        ...
        string Get_NextReading_File()
        {
            ...
            sprintf( sBuf_Tmp, 
                   "D110000zgqnb_%4.4d%02.2d%02.2d_%d-%02.2d.htm", 
                   oNewsPaper.iYear_Newspaper,  oNewsPaper.iMon_Newspaper, oNewsPaper.iDate_Newspaper,
                   this->iXh_Bm_Newspaper, this->iBm_Newspaper );
...
char * CReadNewspaper_ZGQNB::sPaper_Name = "中国青年报" ;
