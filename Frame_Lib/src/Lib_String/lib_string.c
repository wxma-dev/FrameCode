...
{
	char * pPoint_Malloc;           /* char *类型函数返回值 */
	int    iLength_AddStr;          /* 需要添加的字符串的长度 */
	int    iLength_NeedMem;         /* 需要的内存长度 */
	...
	/* 需要的内存长度 = 字符串长度 + 1 */
	iLength_NeedMem = iLength_AddStr + 1;
	
	/* 重新分配内存 */
	pPoint_Malloc = ( char * )malloc( iLength_NeedMem );
	if ( NULL == pPoint_Malloc )
	{
		return NULL;
	}
	
	/* 分配的内存初始化 */
	memset( pPoint_Malloc, 0x00, iLength_NeedMem );

	/* 添加字符串 */
	strcat( pPoint_Malloc, sAdd_String );

	*pDst_Mem = pPoint_Malloc;
	return *pDst_Mem;
}
...
/************************************************************
* Copyright (c) 2015 xxxxxxxxxxxxxxxxxx公司                 *
* Source File:    OutFlag_Kh                                *
* Description:    判断是否是括号，并输出括号类型            *
* Author:         mawx                                      *
* Create Date:    2015/07/09                                *
* In:                                                       *
* In:             sIn_CharKh  括号字符，字符只能输入[,{,(   *
* Out:                                                      *
* Return:         -1 失败                                   *
*                 > 0  成功                                 *
*-----------------------------------------------------------*
...
* Modify Note:    XXXXXXXXXXXXXX                            *
************************************************************/
OutFlag_Kh( const char sIn_CharKh )
{
    if ( '[' == sIn_CharKh || ']' == sIn_CharKh )
    {
        return 1;
    }
    else if ( '{' == sIn_CharKh || '}' == sIn_CharKh )
    {
        return 2;
    }
    else if ( '(' == sIn_CharKh || ')' == sIn_CharKh )
...
/************************************************************
* Copyright (c) 2015 xxxxxxxxxxxxxxxxxx公司                 *
* Source File:    Get_Str_Dkh_Between                       *
* Description:    获取括号之间的字符串                      *
* Author:         mawx                                      *
* Create Date:    2015/07/09                                *
* In:             sIn_Str  需要处理的字符串                 *
* In:             sIn_CharKh  括号字符，字符只能输入[,{,(   *
* Out:            sOut_Str  处理好的字符串                  *
* Return:         NULL 失败或者找不到括号                   *
*                 非NULL  括号结束的地址                    *
*-----------------------------------------------------------*
* Modify Date:    YYYY-MM-DD                                *
* Modifier:       XXX                                       *
* Modify Note:    XXXXXXXXXXXXXX                            *
************************************************************/
int Get_Str_Dkh_Between( const char * sIn_Str, const char sIn_CharKh, char * sOut_Str )
{
    int i = 0;
    int iCout_Start = 0;
    int iCout_End = 0;
    ...
    char cChar_End = 0;
    int iRet = 0;
    int iLocation_Start = 0;
    int iLocation_End = 0;

    iRet = OutFlag_Kh( sIn_CharKh );
    if ( 1 == iRet )
    {
        cChar_Start = '[';
        cChar_End   = ']';
    ...
    int iLength = strlen( sIn_Str ) );
    for( i = 0; i < iLength; i++ )
    {
        if ( cChar_Start == sIn_Str[i] )
        {
            iCout_Start++;
            if ( 1 == iCout_Start )
            {
        ...
        if ( cChar_End == sIn_Str[i] )
        {
            iCout_End++;
        }

        if ( iCout_Start == iCout_End )
        {
            iLocation_End = i;
        }
    }

    /* 没有括号字符或者开头字符与结尾字符不相符合，则认为失败 */
    if ( 0 == iCout_Start || iCout_Start != iCout_End )
    {
        return NULL;
    }

    memcpy( sOut_Str, sIn_Str + iLocation_Start + 1, sIn_Str + iLocation_End );

    return sIn_Str + iLocation_End;
}
...
/************************************************************
* Copyright (c) 2015 xxxxxxxxxxxxxxxxxx公司                 *
* Source File:    Get_Point_Dkh_Between                     *
* Description:    获取括号首位的地址                        *
* Author:         mawx                                      *
* Create Date:    2015/07/09                                *
* In:             sIn_Str  需要处理的字符串                 *
* In:             sIn_CharKh  括号字符，字符只能输入[,{,(   *
* Out:            *ppKhStart  存放首部括号的地址            *
*                 *ppKhEnd  存放尾部括号的地址              *
* Return:         NULL 失败或者找不到括号                   *
*                 非NULL  括号结束的地址                    *
*-----------------------------------------------------------*
...
* Modify Note:    XXXXXXXXXXXXXX                            *
************************************************************/
int Get_Point_Dkh_Between( const char * sIn_Str, const char sIn_CharKh, char ** ppKhStart, char ** ppKhEnd )
{
    int i = 0;
    int iCout_Start = 0;
    int iCout_End = 0;
    ...
    *ppKhStart = sIn_Str + iLocation_Start;
    *ppKhEnd   = sIn_Str + iLocation_End;
